...
type DashboardClientProps = {
    ...
    systemStatus?: {
        ...
        lastSchedulerRunResult?: { published: number; timestamp: string };
        lastOptimizationRun?: string;
        lastOptimizationResult?: { analysis: string; timestamp: string };
    };
    errors: {
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl">
                        ...
                        <div className="mt-4 flex justify-end gap-2">
                            ...
                            <button onClick={handleUpdateDraftText} className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-white">Save</button>
                        </div>
                    </div>
                </div>
            )}
            <div className={`space-y-8 transition-opacity ${isLoadingData ? 'opacity-50' : 'opacity-100'}`}>
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        <Link
                            href={selectedAccount ? `/optimize?accountId=${selectedAccount.id}` : "/optimize"}
                ...
                {errors.accountsError && (
                    <p className="rounded-lg border border-dashed border-border bg-gray-50 p-4 text-sm text-muted-foreground">
                ...
                {accounts.length === 0 ? (
                    ...
                    <>
                        ...
                        <div className="grid gap-4 md:grid-cols-3">
                            ...
                            <div className="rounded-xl border border-border bg-white p-6 shadow-sm">
                                <p className="text-sm font-medium text-muted-foreground">Automation Status</p>
                                <div className="mt-2">
                                    <p className="text-xs text-muted-foreground">Last Check:</p>
                                    <p className="font-medium">
                                        {systemStatus?.lastSchedulerRun
                                            ? DateTime.fromISO(systemStatus.lastSchedulerRun).setZone(DateTime.local().zoneName).toFormat("MM/dd HH:mm")
                                            : "Never"}
                                    </p>
                                </div>
                                {systemStatus?.lastSchedulerRunResult && (
                                    <div className="mt-2">
                                        <p className="text-xs text-muted-foreground">Last Result:</p>
                                        <p className="text-xs">Published: {systemStatus.lastSchedulerRunResult.published}</p>
                                    </div>
                                )}
                                <div className="mt-4 border-t border-border pt-2">
                                    <p className="text-xs text-muted-foreground">Last Optimization:</p>
                                    <p className="font-medium">
                                        {systemStatus?.lastOptimizationRun
                                            ? DateTime.fromISO(systemStatus.lastOptimizationRun).setZone(DateTime.local().zoneName).toFormat("MM/dd HH:mm")
                                            : "Never"}
                                    </p>
                                </div>
                            </div>
                                ...
                                {errors.draftsError ? (
                                    ...
                                    <ul className="space-y-3">
                                        {relevantDrafts.map((draft) => {
                                            ...
                                            return (
                                                <li key={draft.id} className="rounded-lg border border-border bg-gray-50 p-4">
                                                    ...
                                                    <div className="mt-3 flex items-center justify-end gap-2">
                                                        <button onClick={() => handlePublishNow(draft.id)} className="rounded-md bg-primary px-3 py-1 text-xs text-white hover:bg-primary/90">Post Now</button>
                                                        <button onClick={() => openEditModal(draft)} className="rounded-md px-3 py-1 text-xs hover:bg-gray-200">Edit</button>
                                ...
                                {selectedAccount ? (
                                    <>
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className={`rounded-full px-3 py-1 text-xs font-semibold ${selectedAccount.connected ? "bg-blue-100 text-primary" : "bg-gray-100 text-muted-foreground"}`}>
                                        ...
                                        {bestPost ? (
                                            <Link href={bestPost.url ?? "#"} target="_blank" className="block rounded-lg border border-border bg-gray-50 p-4 text-sm transition hover:border-primary">
                                            ...
                                            {recentPosts.length > 0 ? (
                                                <ul className="space-y-2 text-sm text-muted-foreground">
                                                    {recentPosts.map((post) => (
                                                        <li key={post.id} className="rounded-md border border-border bg-gray-50 px-3 py-2">
                                                            ...
                                                            <div className="mt-2 flex flex-wrap gap-2 text-[11px] text-muted-foreground">
                                                                <span className="inline-flex items-center gap-1 rounded-full bg-blue-100 px-2 py-1 font-medium text-primary">Impr {post.metrics.impressions ?? "n/a"}</span>
