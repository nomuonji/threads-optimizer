...
export function AccountTipsControl({ account, onAccountUpdate }: AccountTipsControlProps) {
    ...
    return (
        <div className="space-y-4 rounded-xl border border-border bg-white p-6 shadow-sm relative">
            <div>
                <div>
                ...
                {isLoading ? <p>Loading tips...</p> : (
                    <div className="max-h-60 overflow-y-auto space-y-2 border border-border rounded-md p-2 mt-4">
                        {tips.length === 0 ? <p className="text-sm text-muted-foreground">No tips available in the library.</p> : (
                            tips.map(tip => (
                                <label key={tip.id} className="flex items-start gap-2 p-2 hover:bg-gray-50 rounded cursor-pointer">
                                    <input
                                        type="checkbox"
