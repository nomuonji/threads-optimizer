...
export async function fetchRecentThreadsPosts(
    ...
    for (
        ...
        const params = new URLSearchParams({
            ...
            fields: "id,text,timestamp,media_type,permalink,like_count,reply_count,repost_count,is_reply,reply_to_id,parent_id,replying_to,insights.metric(views,likes,replies,reposts)",
        });
        ...
        let responseData: ThreadsResponse;
        try {
            const res = await fetch(`${THREADS_API_BASE}/${userId}/threads?${params.toString()}`);
            if (!res.ok) {
                const errorText = await res.text();
                throw new Error(`Threads API request failed (status ${res.status}): ${errorText}`);
            }
            responseData = await res.json();
        } catch (error) {
            const err = new Error(
                `Threads API request failed: ${(error as Error).message}`,
        ...
        const nextCursor =
            responseData.paging?.cursors?.after ?? nextCursorFromUrl ?? undefined;
...
export async function publishThreadsPost(
    ...
    if (!containerRes.ok) {
        ...
        throw new Error(`Failed to create Threads media container: ${errorText}`);
    }
