...
import type { AccountDoc, DraftDoc, PostDoc, Platform } from "@/lib/types";
import { fetchTopPosts, fetchRecentPosts } from "@/lib/services/firestore.server";
...
export async function preparePromptPayload(accountId: string, limit = 15) {
    ...
    const [topPosts, recentPosts, drafts] = await Promise.all([
...
export async function generatePost(accountId: string, platform: Platform, limit = 15): Promise<DraftDoc> {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        const prompt = buildPrompt(topPosts, recentPosts, drafts, extraAvoid, account.concept);
