...
import type { AccountDoc, DraftDoc, ExemplaryPost, PostDoc, Tip, Platform, SettingsDoc } from "@/lib/types";
import { fetchTopPosts, fetchRecentPosts, fetchSystemInstruction } from "@/lib/services/firestore.server";
...
export async function preparePromptPayload(accountId: string, limit = 15) {
    ...
    const [topPosts, referencePosts, recentPosts, drafts, tips, exemplaryPosts, systemInstruction] = await Promise.all([
...
export async function generatePost(accountId: string, platform: Platform, limit = 15): Promise<DraftDoc> {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        const prompt = buildPrompt(topPosts, referencePosts, recentPosts, drafts, extraAvoid, tips, exemplaryPosts, account.concept, systemInstruction);
